...
# Sources: # https://ec.europa.eu/eurostat/ (Free access)
#
###############################
...
##########################################################################
#    (1)    Load and clean Eurostat energy balances - Source: # https://ec.europa.eu/eurostat/api/dissemination/sdmx/2.1/data/nrg_bal_s?format=TSV&compressed=false
##########################################################################
...
##########################################################################
#    (2)    Compute price index using electricity, natural gas and oil prices
##########################################################################
...
price_index_eu <- price_index %>%
  filter(cntry_code %in% c('AT', 'BE', 'BG', 'CY', 'CZ', 'DE', 'DK', 'EE', 'ES', 'FI', 'FR',
                           ...
                           'PT', 'RO', 'SE', 'SI', 'SK')) %>%
  group_by(year) %>%
  summarise_at(vars('energy_price_index_estat'), mean, na.rm = TRUE) %>%
  ungroup()
...
nrg_bal_final <- nrg_bal_final %>% dplyr::rename(code2 = cntry_code,
                                TOTAL = total,
                                energy_price_index = energy_price_index_estat) %>%
  select(code2, nace, year, TOTAL, energy_price_index, ENERGY)
...
write.csv(nrg_bal_final, file = 'data/eurostat/klems_energy_fin_eurostat.csv', row.names = FALSE)
