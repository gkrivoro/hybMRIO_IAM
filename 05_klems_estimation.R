# ###################################################################
# 05_klems_estimation.R
# This scripts calculates the elasticities of substitutions and input shares for 
# capital, labor and energy.
# Sources: https://euklems-intanprod-llee.luiss.it/download/ for most up to date EU-KLEMS
# https://dataverse.nl/dataset.xhtml?persistentId=doi:10.34894/FZRMHU for the EUKLEMS 2012 where II are disaggregated across E and M and S.
# ###################################################################
...
path_data_IO <-"C:/Users/boldrin/Sectoral Granularity/repo_final/data/IO/EXIOBASE/IOT_2022_ixi/"
path_data_KLEMS <-"C:/Users/boldrin/Sectoral Granularity/repo_final/data/macro/euklems/"


path_data_energy <-"C:/Users/boldrin/Sectoral Granularity/repo_final/data/eurostat/"

run_id<-"2024-07-17_09-37"
...
##########################################################################
#    (1)    Load and clean EU-KLEMS data
##########################################################################
...
dt.energy.prices<-fread(paste0(path_data_energy,"klems_energy_fin_eurostat.csv"))
...
## Convert sectors from euklems to exiobase
...
#####
####Add energy share present only in old data (2010 )
#####
...
###Basically only the US has sectoral energy share data
...
##### Adding derived price differences before the summation to avoid some kind of jensens inequality effect.
...
## Take the log to perform the estimations
...
##########################################################################
#    (2)    Compute elasticities of substitutions and factor shares. This 
#           is performed in two ways:
#         1) country-by-country regressions with computation of the percentiles of the coefficients. 
#         This assumes that countries may have different production possibility frontiers. 
#         2) Pool countries and compute a clustered SD. This assumption is based on the idea that all the
#           countires are on the same production possibility frontier but that variability could differ between countries (option used in the paper)
##########################################################################




getEOS<-function(dat,y,x,segment1,segment_var=NULL,outlier_bounds=2, N_min=50,cluster=0){
  list.res<-list()
  if (is.null(segment_var)|cluster==1){
    for (mysegment1 in unique(dat[,get(segment1)])){
      ...
      if (nrow(dat.estimation)>N_min){
        if (cluster == 0){
          ...
          results1<-feglm(as.formula(paste0(y,"~",x)) , family="gaussian",data=dat.estimation, weights=dat.estimation[,GO_Q])
          dt.temp<-data.table(coefficients = results1$coefficients,sd =summary(results1, cluster = "country")$coeftable[,2], 
                              varnames = names(results1$coefficients),segment1=mysegment1)
          setnames(dt.temp,"segment1",segment1)
          list.res[[mysegment1]]<-dt.temp
...
sum.EM_Y[,.(signif(p10,2),signif(p25,2),signif(p50,2),signif(p75,2))]
...
fwrite(sum.E_EM,paste0(path_run_out,"sum.E_EM.ctry.csv"))
fwrite(sum.EM_Y,paste0(path_run_out,"sum.EM_Y.ctry.csv"))
fwrite(sum.KL_Y,paste0(path_run_out,"sum.KL_Y.ctry.csv"))
fwrite(sum.E_Y,paste0(path_run_out,"sum.E_Y.ctry.csv"))

#####################
# now do clustered SD
#####################
out1.E_EM_cluster<-getEOS(dt.ykl1.cprs,"D1.log.E_EM_q","D1.log.E_EM_p","sector.eos","country", cluster=1)
out1.EM_Y_cluster<-getEOS(dt.ykl1.cprs,"D1.log.EM_Y_q","D1.log.EM_Y_p","sector.eos","country", cluster=1)
out1.E_Y_cluster<-getEOS(dt.ykl1.cprs,"D1.log.E_Y_q","D1.log.E_Y_p","sector.eos","country", cluster=1)
out1.KL_Y_cluster<-getEOS(dt.ykl1.cprs,"D1.log.KL_Y_q","D1.log.KL_Y_p","sector.eos","country", cluster=1)


fwrite(out1.E_EM_cluster[,.(sector.eos,EOS, sd_D1.log.E_EM_p, EOS_low95conf, EOS_high95conf)],
       paste0(path_run_out,"sum.E_EM.cluster.csv"))
fwrite(out1.EM_Y_cluster[,.(sector.eos,EOS, sd_D1.log.EM_Y_p, EOS_low95conf, EOS_high95conf)],
       paste0(path_run_out,"sum.EM_Y.cluster.csv"))
fwrite(out1.E_Y_cluster[,.(sector.eos,EOS, sd_D1.log.E_Y_p, EOS_low95conf, EOS_high95conf)],
       paste0(path_run_out,"sum.E_Y.cluster.csv"))



unique(dt.ykl1.cprs[,country])

library(xtable)
?xtable
print(xtable(out1.E_EM_cluster[,.(sector.eos,EOS, sd_D1.log.E_EM_p, EOS_low95conf, EOS_high95conf)], type = "latex",digits=3))

print(xtable(out1.EM_Y_cluster[,.(sector.eos,EOS, sd_D1.log.EM_Y_p, EOS_low95conf, EOS_high95conf)], type = "latex",digits=3))
print(xtable(out1.E_Y_cluster[,.(sector.eos,EOS, sd_D1.log.E_Y_p, EOS_low95conf, EOS_high95conf)], type = "latex",digits=3))
...
fwrite(x1.cprs.shares.all,paste0(path_run_out,"klems_shares.csv"))
