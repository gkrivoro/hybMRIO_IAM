####################################################################
# 06_output_euklems.R
# Computes back of the envelop calculations for the projections 
# of output quantities (price paribus) using clustered SD.
# 
####################################################################
...
#############################################################################
# 1) Load KLEMS shares and capital stock/labor levels For certain very small sectors in small countries, we may be missing capital and labor. 
#     For output=0 country-sectors, we're missing shares.
#############################################################################

klems_shares<-fread(paste0(path_run_out,"klems_shares.csv"))
...
sum.E_EM<-fread(paste0(path_run_out,"sum.E_EM.cluster.csv"))
sum.EM_Y<-fread(paste0(path_run_out,"sum.EM_Y.cluster.csv"))
sum.E_Y<-fread(paste0(path_run_out,"sum.E_Y.cluster.csv"))
xwalk.eos_sectors<-fread(paste0(path_input,"EOS_supersectors.csv"))


#############################################################################
# 2) Merge KLEMS shares and do initial calculations without NiGEM deltas. 
...
#############################################################################
# 3) - Here we E and M are complements due to lack of EQ reaction again. So use E_Y elasticity. 
#
...
#############################################################################
# 4) the lack of sensitivity is due to the fact that the factors are near complements in the outer nest. 
#
...
dt.all.eos.ts$variable <- plyr::mapvalues(dt.all.eos.ts$eos_type, from=c('series.out1', 'series.nested.mean', 'series.nested.high', 'series.nested.low', 
                                         'series.out.2.mean', 'series.out.2.high', 'series.out.2.low'),
          to=c('Output - Energy Only', 'Output - (KL)(EM) mean', 'Output - (KL)(EM) p97.5', 'Output - (KL)(EM) p2.5', 
               'Output - (KL)E mean', 'Output - (KL)E p97.5', 'Output - (KL)E p2.5'))


dt.all.eos.ts[,value.2020:=value[year==2020],.(model.short,scenario, country, sector, year, eos_type,value)]

sectoral_output <- dt.all.eos.ts %>% group_by(model.short, scenario, country,
                           sector, variable) %>% 
  ...
  mutate(Model = dplyr::case_when(
    model.short == "gcam" ~ "GCAM 6.0 NGFS",
    model.short == "remind" ~ "REMIND-MAgPIE 3.3-4.8",
    model.short == "message" ~ "MESSAGEix-GLOBIOM 2.0-M-R12-NGFS",
    TRUE ~ model.short  
  )) %>% 
  pivot_wider(id_cols = c('Model', 'country',
                          'sector', 'year','variable'),
              names_from = scenario, values_from = value.index) 


sectoral_output$Unit <- 'Quantity index (2020 = 1)'

sectoral_output %>% write.csv(paste0(path_run_out, '/dt_all_eos_ts.csv'), row.names = F)
  
final_energy_series <- fread(paste0(path_run_out,"final_energy_series.csv" )) %>% 
  dplyr::rename(final_energy = series,
                scenario = Scenario) %>%
  select(country, sector, model.short, scenario, year, final_energy) %>% 
  mutate(Model = dplyr::case_when(
    model.short == "gcam" ~ "GCAM 6.0 NGFS",
    model.short == "remind" ~ "REMIND-MAgPIE 3.3-4.8",
    model.short == "message" ~ "MESSAGEix-GLOBIOM 2.0-M-R12-NGFS",
    TRUE ~ model.short)) %>% 
  pivot_wider(id_cols = c('Model', 'country',
                          'sector', 'year'), names_from = scenario, values_from = final_energy)


final_energy_series$variable <- 'Final energy'
final_energy_series$Unit <- 'EJ/yr'



rbind(sectoral_output, final_energy_series) %>% write.csv(paste0(path_run_out, '/sectoral_output_phaseV.csv'), row.names = F)
  


## Export data for IIASA Portal

rbind(sectoral_output, final_energy_series) %>% 
  pivot_longer(cols = "Below 2°C":"Net Zero 2050",
               names_to = 'Scenario',
               values_to = 'Value') %>% 
  pivot_wider(values_from = 'Value', names_from =  'year')  %>% 
  mutate(Variable = paste(sector, variable, sep = '|')) %>% 
  
  dplyr::rename(Region = country ) %>% 
  select(Model, Scenario, Region, Variable, Unit, `2020`:`2100`)%>% 
  write.csv(paste0(path_run_out, '/sectoral_output_phaseV_IIASA.csv'), row.names = F)
