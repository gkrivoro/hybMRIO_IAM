...
#   
#
#         in  version 8 we build on version 6 by disaggregating the "other" sector. 
#           this new crosswalk is created using disaggregate_9other.R, using a text file called 9other_reclass.csv
#           We also use phase IV forecasts here instead as well. 
#
...
#save for output BoE calculation
...
###This is needed for Back of Envelope output calculations,
#     but saving it every run might balloong in size.
...
##########################################################################
#  (5)      Entering into the main calculations
...
##### [added 11/13/23] new input for phase IV scenarios. 
#####[added 11/22/23] replacement series for missing message and remind sectors.
dt.iam.series<-loadiam(paste0(path_input, "231122_1535_extract.csv"))
...
# 
# dt.filler<-unique(dt.iam.series[,.(Model, Scenario, series, Unit,  model.short, scenario.short, Region)])
# dt.filler[,ind:=1:.N]
...
dt.iam.series[model.short=="remind"&series=="Final Energy|Industry|Non-ferrous metals"]
dt.iam.series[model.short=="message"&series=="Final Energy|Industry|Chemicals"]

#remind is missing non-ferrous metals. 
#               . So just put in regular industry series there. 
#         [edit 11/13] MESSAGE is no longer missing chemicals as of phase IV
#################################
#     Fill in higher level series when other is unavailable.
#         In Phase IV, these are
#        For Remind :   Final Energy|Industry|Non-ferrous metals  <- Final Energy|Industry
#                       Final Energy|Transportation|Aviation  <- Final Energy|Transportation|Aviation|Passenger
#         For Message:  Final Energy|Transportation|Rail+Road <-Final Energy|Transportation
#                       Final Energy|Transportation|Maritime <-Final Energy|Transportation
#                       Final Energy|Transportation|Aviation <-Final Energy|Transportation
...
unique(dt.iam.series[model.short=="message",series])
...
# country_iam_xwalk.long[country.exiobase=="AU"]
# unique(dt.iam.series[model.short=="remind",Region])
...
##########################################################################
# (7) This is for primary and secondary energy series
##########################################################################
...
#fwrite(model_xwalk,paste0(path_output,"model_xwalk.csv" ))
...
mycountry<-"AU"
#something is wrong with 3-energy-intensive|fertilisers and agrochemicals
#       this is due to some issue with the message final energy|chemicals series. 
...
graph_sector_iam.2(dt.netzero2050min2020,country_iam_xwalk,model_xwalk,
                   mycountry,"Final Energy, Net Zero vs. Current Scenario, % Diff. in 2050","netzero.ov.current.energy.2050","my.series.2.over.1","% difference",
                   paste0(path_graphs,"overview/paper/" ),N,gsub("\\|","\\\\|",set_sectors.IO),0)
