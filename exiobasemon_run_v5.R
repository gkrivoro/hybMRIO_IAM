...
#
#     In v5 we use analytical solution
...
path_graphs <-paste0(path_wd,"graphs/" )
...
##Make the industry crosswalks
# define the Energy sectors. I set these as the first sectors in the list. 
EnergySectors<-c("1-fossil|coal","1-fossil|oil", "1-fossil|gas","2-utility|electricity|fossil|coal","2-utility|electricity|fossil|oil", "2-utility|electricity|fossil|gas","2-utility|electricity|renewable","2-utility|electricity|transmission") 
#for now, just set the countries to the default here. But, can take a subset for the actual NiGEM set. 
countries.nigem<-unique(x1[,region])

N_country<-length(countries.nigem)


industries.nigem<-gen_industry_tables(paste0(path_input, "CPRS_Xwalk_v3.csv"), EnergySectors, countries.nigem)



unique(industries.nigem$industry.tab[,sector])
##########################################################################
#        Loading IAM series and IAM-CPRS concordance
##########################################################################

#load the IAM inputs. They're classified as being standard, upstream and downstream. 
#CPRS_IAM<-fread(paste0(path_input,"/cprs_sector_iam_v2.csv"))
CPRS_IAM<-fread(paste0(path_input,"/cprs_sector_iam_v3.csv"))

country_iam_xwalk<-fread(paste0(path_input,"/country_IAM_xwalk.csv"))



#self.profile is the transition risk profile the sector has. However, the direct equating of sectors done through sector.iam. 
industry.iam.tab<-unique(industries.nigem$industry.tab[,.(sector_id, sector)])
industry.iam.tab<-merge(industry.iam.tab, CPRS_IAM[,.(sector,sector.iam, I_primsecenergy.iam=I_primsecenergy)] ,by="sector")
industry.iam.ctry.tab<-merge(industries.nigem$industry.cntry.tab,industry.iam.tab[,.(sector_id,sector.iam, I_primsecenergy.iam)] , by="sector_id")
...
##########################################################################
#        generate Leontief inverse using nigem/cprs sectors. Also make ghosh inverse
##########################################################################

x1.small<-parse_exiobase_x(paste0(path_data, "x.txt"), industries.nigem)
Z.long<-parse_exiobase_Z(paste0(path_data, "Z.txt"),2, NULL,industries.nigem)
N<-length(unique(industries.nigem$industry.tab[,sector]))

dt.alpha_ij<-gen_alpha(Z.long, x1.small, industries.nigem)

dt.ghosh_ij<-gen_ghosh(Z.long, x1.small, industries.nigem)


rm(Z.long)

#rm(Z.long)
#some sectors had 0 output and so we need to set requirements there to 0. 
dt.alpha_ij[is.nan(coef),coef:=0]
dt.ghosh_ij[is.nan(coef),coef:=0]


dt.b_ij<-gen_invdt(dt.alpha_ij,industries.nigem)
dt.G_ij<-gen_invdt(dt.ghosh_ij,industries.nigem)




##########################################################################
#        Load satellite accounts with energy balances.
##########################################################################


#dt.b_ij[country.out=="US"&sector.out=="1-fossil|coal"][order(-value),.(country.out, sector.out, country.in, sector.in, value)]

#[country.out=="US"&sector.out=="3-energy-intensive"][order(-value),.(country.out, sector.out, country.in, sector.in, value)]
...
##########################################################################
# Get energy accounts
F.sat.long<-parse_exiobase_F(paste0(path_data, "/satellite/F.txt"),2, "stressor",energy_balances,industries.nigem)

F.sat.long<-merge(F.sat.long, industries.nigem$industry.cntry.tab[,.(id.out=id,sector.out=sector,country.out =country)],by="id.out")
...
##########################################################################
#       loading IAM series and merging them with the weights.
##########################################################################
...
#for now we will just match everything to "world"

#dt.iam.series<-loadiam(paste0(path_input, "ngfs_snapshot_1675705596.csv"))
dt.iam.series<-loadiam(paste0(path_input, "ngfs_snapshot_1682365836.csv"))

dt.iam.series.remind.mess.transport<-loadiam(paste0(path_input, "ngfs_message_remind_transport.csv"))
...
#########################################
#message is missing separate aviation, rail+road, and maritime. 
#remind aviation is aviation|passenger. Just use the other transport industries as proxies then. 
#Level doesn't mater since they'll be readjusted according to IO table.


dt.iam.new.trans.series<-rbind( dt.iam.series.remind.mess.transport[series=="Final Energy|Transportation|Aviation|Passenger"&model.short=="remind",
                                           .(Model, Scenario, Unit, year, model.short, scenario.short, Region, value,series= "Final Energy|Transportation|Aviation")],
                        dt.iam.series.remind.mess.transport[series=="Final Energy|Transportation"&model.short=="message",
...
# 
# 
# #need to interpolate for every 5 years or else there will be spikes. 

dt.filler<-unique(dt.iam.new.trans.series[,.(Model, Scenario, series, Unit,  model.short, scenario.short, Region)])
dt.filler[,ind:=1:.N]
dt.filler2<-CJ(ind=dt.filler[,ind],year=c(2065,2075,2085,2095))
dt.filler<-merge(dt.filler, dt.filler2,by="ind")
dt.filler[,value:=NA]
dt.filler[,ind:=NULL]
dt.iam.new.trans.series<-rbind(dt.iam.new.trans.series,dt.filler)

#remind is missing non-ferrous metals and message is missing chemicals. So just put in regular industry series there. 
dt.iam.new.industry<-rbind( dt.iam.series[series=="Final Energy|Industry"&model.short=="remind",
                                                                    .(Model, Scenario, Unit, year, model.short, scenario.short, Region, value,series= "Final Energy|Industry|Non-ferrous metals")],
                            dt.iam.series[series=="Final Energy|Industry"&model.short=="message",
                                                                    .(Model, Scenario, Unit, year, model.short, scenario.short, Region,value, series= "Final Energy|Industry|Chemicals")])
                               
                      

dt.iam.series<-rbind(dt.iam.series, dt.iam.new.trans.series)
dt.iam.series<-rbind(dt.iam.series, dt.iam.new.industry)

#table(dt.iam.series[,series],dt.iam.series[,model.short])

#########################################

#Getting 2020 output to downscale with. 
...
dt.iam.series<-dt.iam.series[scenario.short!="ndc"]
...
unique(dt.iam.series[model.short=="gcam",Region])

##########################################################################
# 1) For primary and secondary energy series, we take the downstream energy intensity as the OUTPUT
##########################################################################
...
##########################################################################
# (2) For other series that are modeled by the forward system (IAM), we take the upstream energy intensity as the energy series in question.
#     We assume energy intensity remains constant but usage goes down or up according to IAM index. So you can think of this as an energy series indexed to 2020
...
if (I_graph_all==1){
  ...
  for (mysec in unique(dt.standalone.iam[,sector])){
    # for (mycountr in unique(industry.iam.ctry.tab[,country])){
    #mycountry<-"US"
    #mysec<-"2-utility|electricity|fossil|gas"
    print(mycountry)
    print(mysec)
...
##########################################################################
# (3b) Derive (1-A_{IO, IO})^-1
##########################################################################


#create inverse block matrix for IO. 

industries.nigem.IO<-copy(industries.nigem)
industries.nigem.IO$industry.tab<-industries.nigem$industry.tab[sector%in%sector.other]
industries.nigem.IO$industry.cntry.tab<-industries.nigem$industry.cntry.tab[sector%in%sector.other]

dt.b_ij_IO<-gen_invdt(dt.alpha_ij[sector.in%in%sector.other&sector.out%in%sector.other],industries.nigem.IO)


##########################################################################
# (3c) Derive A_{IAM,IO}(1-A_{IO, IO})^-1
#this is to help with decompositions. 
##########################################################################

countries<-unique(industries.nigem.IO$industry.cntry.tab[,country])
list.L_0<-list()
...
for (mycountry in countries){
  print(mycountry)
  #dt.b_ij.standalone.in<-dt.b_ij.tilde.energy[(I_primsecenergy.iam.in==0|I_primsecenergy.iam.in==1)&country.out==mycountry]
  ...
  #list.scope3.iam[[mycountry]]<-dt.b_ij.tilde.iam[,.(series=sum(value*TJperMeuro.in*output.iam.downscale)),
  #                                           by=c("id.out","country.out","sector.out","model.short", "scenario.short","Model","Scenario" ,"year")]
...
dt.alpha_IAMIO_b_IOIO<-merge(dt.alpha_IAMIO_b_IOIO, industries.nigem$industry.cntry.tab[,.(id.in.iam=id,sector.in=sector, country.in=country)],by="id.in.iam")
dt.alpha_IAMIO_b_IOIO<-merge(dt.alpha_IAMIO_b_IOIO, industries.nigem$industry.cntry.tab[,.(id.out.IO=id,sector.out=sector, country.out=country)],by="id.out.IO")
...
for (mycountry in countries){
  ...
  list.L_0[[mycountry]]<-merge( list.L_0[[mycountry]], industries.nigem$industry.cntry.tab[,.(id.out.IO=id,sector.out=sector, country.out=country)],by="id.out.IO")
  ...
  dt.L_01_topwt.alt<-merge( dt.L_01_topwt.alt, industries.nigem$industry.cntry.tab[,.(id.out.IO=id,sector.out=sector, country.out=country)],by="id.out.IO")
  dt.L_01_topwt.alt<-merge( dt.L_01_topwt.alt, industries.nigem$industry.cntry.tab[,.(id.in=id,sector.in=sector, country.in=country)],by="id.in")
...
dt.E3_IO2<-dt.E3_IO2[scenario.short%in%c("current","delayed","netzero")]
dt.decomp<-dt.decomp[scenario.short%in%c("current","delayed","netzero")]
#dt.decomp[rank>5, sector.in=]
...
graphseries.nigem.3(dt.E3_IO,dt.decomp,"message","9-other","DE","series",paste0(path_graphs,"monetary/IO/"),0)
...
if (I_graph_all==1){
    for (mysec in unique(dt.E3_IO[,sector])){
      ...
      for (mymod in unique(dt.E3_IO[,model.short])){
        ...
        print(mysec)
        print(mymod)
        graphseries.nigem.3(dt.E3_IO,dt.decomp,mymod,mysec,mycountry,"series",paste0(path_graphs,"monetary/IO/"),0)
      }
    }
  }
}

graphseries.nigem.3(dt.E3_IO,dt.decomp,"gcam","7-finance","US","series",paste0(path_graphs,"monetary/IO/"),0)


#combine this with standalone series
...
fwrite(dt.all, paste0(path_output,"final_energy_series.csv" ))




####################################################################
# Make an output back of the envelop calc
# Here, load energy intensity series that I generated in energy_intensity.R
####################################################################
dt.int1<-fread(paste0(path_output,"dt.enintensity.csv"))


dt.alpha_ij.energy<-merge(dt.alpha_ij,dt.ups.energy.sum[,.(id.in=id.out, TJperMeuro.in.sum)],by="id.in")
dt.alpha_ij.energy<-dt.alpha_ij.energy[order(id.out, id.in)]
#dt.alpha_ij.energy[,coefpTJ:=coef/TJperMeuro.in.sum]

electricity_sectors<-c("2-utility|electricity|fossil|gas","2-utility|electricity|fossil|oil","2-utility|electricity|fossil|coal","2-utility|electricity|renewable")

electric_energy<- dt.ups.energy.sum[sector.out%in%electricity_sectors, 
                                    .(TJperMeuro_electric=mean(TJperMeuro.in.sum)), .( country.out)][,.(country.out,TJperMeuro_electric)]

dt.alpha_ij.energy<-merge(dt.alpha_ij.energy,electric_energy,by="country.out")




dt.alpha_ij.energy[,coef.wtTJ:=coef*TJperMeuro.in.sum/TJperMeuro_electric, .(id.out)]



dt.energyshare1<-dt.alpha_ij.energy[,.(alpha=sum(coef)), .(id.out, country.out, sector.out)]


dt.energyshare_ren<-dt.energyshare1[sector.out=="2-utility|electricity|renewable"]
dt.energyshare_gas<-dt.energyshare1[sector.out=="2-utility|electricity|fossil|gas"]
dt.energyshare_ren1<-merge(dt.energyshare_ren,dt.energyshare_gas[,.(country.out, alpha.gas=alpha)],by="country.out")
dt.energyshare_ren
dt.energyshare_mod<-rbind(dt.energyshare1[sector.out!="2-utility|electricity|renewable"],dt.energyshare_ren1[,.(country.out, id.out, sector.out, alpha=alpha.gas)])
...
dt.all.out<-merge(dt.all, dt.energyshare_mod[,.(id=id.out, alpha)] ,by="id")
dt.all.out[,Omega.2020 := gross_output.2020^(1-alpha)*series.2020^(alpha)]
dt.all.out[,series.out :=Omega.2020 * (series*gross_output.2020)^alpha]


fwrite(dt.all.out[,.(id, country, sector, region.iam, Model, Scenario, model.short, scenario.short, year, series, series.out)], paste0(path_output,"final_energy_series_out.csv" ))
...
dt.all[,series.2050.current:=series[year==2050&scenario.short=="current"],.(model.short,  id,country,sector)]
dt.all[,series.2050.netzero:=series[year==2050&scenario.short=="netzero"],.(model.short,  id,country,sector)]




dt.2050.current<-dt.all[year==2050&scenario.short=="current",.(model.short,series  ,id,country,sector)]
dt.2050.netzero<-dt.all[year==2050&scenario.short=="netzero",.(model.short, series, id,country,sector)]

dt.2050.netzero.current<-merge(dt.2050.netzero, dt.2050.current[,.(model.short, series.current=series, id,country,sector)], by=c("model.short" ,"id","country","sector"))
dt.2050.netzero.current[,metric:=(series/series.current-1)*100]

dt.all[,series.2050.netzero.ov.current:=(series.2050.netzero/series.2050.current-1)*100,.(model.short, scenario.short, id,country,sector)]


dt.all[,series.2050ov2020:=(series.2050/series.2020-1)*100,.(model.short, scenario.short, id,country,sector)]

dt.all.sum<-dt.all[,.(D1.series.sum.2020=sum(D1.series[year>2020]),
                      D1.series.mean.2020=mean(D1.series[year>2020]),
                      D1.series.mean.2020.2050=mean(D1.series[year>2020&year<=2050]),
                      D1.series.sum.2020.2050=sum(D1.series[year>2020&year<=2050]),
                      series.2050ov2020=mean(series.2050ov2020),
                      series.2050.netzero.ov.current= mean(series.2050.netzero.ov.current),
                      D1.series.sum.2025.2050=sum(D1.series[year>2025&year<=2050])),.(model.short, scenario.short, id,country,sector)]


graph_sector_iam(dt.all.sum,country_iam_xwalk, model_xwalk,"GB","current", "series.2050.netzero.ov.current","2020-2050 % change in TJ",paste0(path_graphs,"monetary/overview/" ),1)
...
if (I_graph_all ==1){
  for (mycountry in unique(industry.iam.ctry.tab[,country])){
    ...
    graph_sector_iam(dt.all.sum,country_iam_xwalk,model_xwalk,
                     mycountry,"netzero","series.out.2050ov2020","2020-2050 % change in output",
                     paste0(path_graphs,"monetary/output/" ),1)
